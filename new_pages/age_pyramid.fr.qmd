# Pyramides démographiques et échelles de Likert {#age\_pyramid}
...
Ce morceau de code montre le chargement des paquets nécessaires aux analyses. Dans ce manuel, nous mettons l'accent sur `p_load()` de **pacman** qui installe le paquet si nécessaire *et* le charge pour l'utiliser. Tu peux aussi charger les paquets installés avec `library()` à partir de **base** R. Voir la page sur [Les bases du R](basics.qmd) pour plus d'informations sur les paquets R.
...
Tu trouveras ci-dessous un morceau de code de test.

```{r}
a <- 2 + 2       # basic addition
b <- a - 2 * 3   # doing something with the outcome

a   # print the outcome of A
...
Importe la liste de cas nettoyée d'une épidémie d'Ebola simulée. Pour suivre l'évolution de la situation, <a href='https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'>clique pour télécharger la liste "propre".</a> (sous forme de fichier .rds). Importe les données à l'aide de l'outil `import()` à partir de la fonction **rio** (elle gère de nombreux types de fichiers tels que .xlsx, .csv, .rds - voir la rubrique [Importer et exporter](importing.qmd) pour plus de détails).
...
Pour la démonstration, nous regroupons les données de la liste des lignes en nombres par catégorie d'âge et par sexe, dans un format "large". Cela permet de simuler comme si tes données étaient d'abord sous forme de nombres. En savoir plus sur [Regrouper les données](grouping.qmd) et [Pivoter les données](pivoting.qmd) dans leurs pages respectives.
...
Note que l'ordre des facteurs "m" et "f" est différent (pyramide inversée). Pour ajuster l'ordre, tu dois redéfinir le sexe dans les données agrégées comme un facteur et ordonner les niveaux comme tu le souhaites. Voir le [Facteurs](factors.qmd) page.
...
Importe et visualise les données de la population (voir [Télécharger le manuel et les données](#data-used) page) :
...
Importe les données (voir [Télécharger le manuel et les données](#data-used) si tu le souhaites).
